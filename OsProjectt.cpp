...
int main()
{
	...
	printf("So per process you will give only %d of tim         \n",per);
	//sum=0;
	...
	if(po==1)
	{
		...
		for(int i=0;i<op;i++)
		{
			printf("\n Enter the ARRIVAL tim and BUST time %d  ",i+1);
			scanf("%d %d",&a[i],&b[i]);
			t:if(a[i]>cutt)
			{
		...
		printf("\n\n Sudesh Ur new Per process time is %d\n",per);
		m3:
	int x=tim;
	for(int i=0;i<op;i++)
	{
	...
	{
	printf("\n Average Waiting Time is %d       ",float(waitf+waits+waitff+waitfs)/totalp);
	printf("\n Average processing time is %d    ",float(tim/totalp));
	int kj;
	cin>>kj;
	exit(0);
	}
	...
	if(tim>=120)
	{
	printf("\n Average Waiting Time is %d     ",(waitf+waits+waitff+waitfs)/totalp);
	printf("\n Average processing time is %d  ",tim/totalp);
	exit(0);
	}
	else
	{
